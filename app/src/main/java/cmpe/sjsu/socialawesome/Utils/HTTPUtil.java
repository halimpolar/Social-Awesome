...
public class HTTPUtil {
    ...
    public static void sendPushNotification(Context context, List<String> tokens, String title, String message, Map<String, String> data) {
        sendPushNotification(context, new PushMessageResquest(tokens, title, message, data));
    }

    static void sendPushNotification(final Context context, PushMessageResquest resquest) {
        ...
        try {
            ...
            smRequestQueue.add(new JsonObjectRequest(PUSH_NOTIFICATION_URL, jsonBody, new Response.Listener<JSONObject>() {
                @Override
                public void onResponse(JSONObject response) {
                    Toast.makeText(context, context.getString(R.string.success_send_push), Toast.LENGTH_SHORT).show();
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Toast.makeText(context, context.getString(R.string.failure_send_push), Toast.LENGTH_SHORT).show();
                }
            }));
